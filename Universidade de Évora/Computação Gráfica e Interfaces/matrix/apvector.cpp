...
template <class itemType> itemType & apvector<itemType>::operator [] (int k)
...
template <class itemType> void apvector<itemType>::resize(int newSize)
...
template <class itemType>
apvector<itemType> operator + (const apvector<itemType> & lhs, const apvector<itemType> & rhs)
	{
   int vec_len = lhs.length() + rhs.length();
   apvector<itemType> temp(vec_len);
   int x;
   for(x=0; x < lhs.length(); ++x)
   	temp[x] = lhs[x];
   for(int y=0; y < rhs.length(); ++y)
   	temp[(x+y)] = rhs[y];
   return temp;
   }
